...
{
    public class UserCosmosRepositoryTest
    {
        [Fact]
        ...
        [Fact]
        public async Task get_by_id_should_invoke_get_async_on_repository()
        {
            var userRepository = new Mock<ICosmosRepository>();
            userRepository.Setup(x => x.GetById(It.IsAny<Guid>())).ReturnsAsync(new User());
            var mapper = new Mock<IMapper>();
            var passwordService = new Mock<IPasswordService>();
            var userService = new UserService(userRepository.Object, passwordService.Object, mapper.Object);
            await userService.Get(Guid.Parse("75994b75-5412-419e-8fb6-bd22e930fca8"));
            userRepository.Verify(x => x.GetById(It.IsAny<Guid>()), Times.Once);
        }

        [Fact]
        public async Task get_by_email_should_invoke_get_async_on_repository()
        {
            var userRepository = new Mock<ICosmosRepository>();
            userRepository.Setup(x => x.GetByEmail(It.IsAny<string>())).ReturnsAsync(new User());
            var mapper = new Mock<IMapper>();
            var passwordService = new Mock<IPasswordService>();
            var userService = new UserService(userRepository.Object, passwordService.Object, mapper.Object);
            await userService.Get("krzysztof.lach@icloud.com");
            userRepository.Verify(x => x.GetByEmail(It.IsAny<string>()), Times.Once);
        }
    }
}
